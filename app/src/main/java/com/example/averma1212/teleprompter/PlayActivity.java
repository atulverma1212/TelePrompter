...
public class PlayActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(startingIntent.hasExtra(SCRIPTS)){
            script = startingIntent.getStringExtra(SCRIPTS);
        }
    ...
    private void setupSharedPref() {
        ...
        String color = sharedPreferences.getString(getString(R.string.pref_text_color_key),
                getString(R.string.pref_color_black_value));
        playScript.setTextColor(getColor(color));
        color = sharedPreferences.getString(getString(R.string.pref_bg_color_key),
        ...
        playScript.setTypeface(Typeface.createFromAsset(getAssets(),"fonts/" +
                font.toUpperCase() + ".ttf"), 1);
        if(isMirrored){
